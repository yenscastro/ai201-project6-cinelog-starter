...
def sample_film(app):
    ...
    with app.app_context():
        film = Film(title="The Godfather", year=1972, genre="Drama")
        db.session.add(film)
        db.session.commit()
        return film.id
...
def test_add_to_watchlist_creates_entry(app, sample_user, sample_film):
    ...
    with app.app_context():
        ...
        assert entry.public == True  # Default
...
def test_get_watchlist_returns_newest_first(app, sample_user):
    ...
    with app.app_context():
        ...
        film_a = Film(title="Citizen Kane", year=1941, genre="Drama")
        film_b = Film(title="Casablanca", year=1942, genre="Romance")
        ...
        # Casablanca was added later, so it should come first
        assert titles[0] == "Casablanca"
        assert titles[1] == "Citizen Kane"